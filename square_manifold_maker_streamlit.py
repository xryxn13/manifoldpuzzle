...
st.markdown('<div class="subtitle">The goal is to fold the paper so that only one image is visible on one side of the paper, and the other image is visible on the other side. The cells marked with cross should be hidden. You can fold the paper however you want, but you cannot tear it.</div>', unsafe_allow_html=True)

# Level selection
st.markdown('<div class="section">📊 Select Difficulty Level:</div>', unsafe_allow_html=True)
level = st.radio("", ["Easy", "Medium", "Hard"], horizontal=True)
